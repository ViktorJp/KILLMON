...
paranoidmode () {
  ...
  #iptables -I FORWARD -i br+ -o eth0 -j $FW_CHAIN
  #iptables -A $FW_CHAIN -i br+ -o eth0 -j REJECT
...
paranoidmode6 () {
  ...
  #ip6tables -I FORWARD -i br+ -o eth0 -j $FW_CHAIN
  #ip6tables -A $FW_CHAIN -i br+ -o eth0 -j REJECT
...
iprangemode () {
  ...
  #iptables -I FORWARD -i br+ -o eth0 -j $FW_CHAIN
  #iptables -A $FW_CHAIN -s 192.168.1.0/24 -o eth0 -j REJECT
  #iptables -A $FW_CHAIN -m iprange --src-range 192.168.1.235-192.168.240 -j REJECT
...
iprangemode6 () {
  ...
  if [ $ip6rangenotation -eq 0 ]; then
    if [ $WANIFUSED -eq 0 ]; then
      ip6tables -I FORWARD -i br+ -o $WAN0IFNAME -j $FW6_CHAIN
      ip6tables -A $FW6_CHAIN -s $iprangecidr -o $WAN0IFNAME -j REJECT
    elif [ $WANIFUSED -eq 1 ]; then
      ip6tables -I FORWARD -i br+ -o $WAN1IFNAME -j $FW6_CHAIN
  ...
  #ip6tables -I FORWARD -i br+ -o eth0 -j $FW_CHAIN
  #ip6tables -A $FW_CHAIN -s 192.168.1.0/24 -o eth0 -j REJECT
  #ip6tables -A $FW_CHAIN -m iprange --src-range 192.168.1.235-192.168.240 -j REJECT
...
singleipmode () {
  ...
  #iptables -I FORWARD -i br+ -o eth0 -j $FW_CHAIN
  #iptables -A $FW_CHAIN -s 192.168.1.235 -o eth0 -j REJECT
...
singleipmode6 () {
  ...
  #ip6tables -I FORWARD -i br+ -o eth0 -j $FW_CHAIN
  #ip6tables -A $FW_CHAIN -s 192.168.1.235 -o eth0 -j REJECT
...
reverserules () {
  ...
  # Reverse Paranoid Mode iptables rules
  #iptables -D $FW_CHAIN -i br+ -o eth+ -j REJECT
  # Reverse IP Range Mode iptables rules
  #iptables -D $FW_CHAIN -s 192.168.1.0/24 -o eth0 -j REJECT
  #iptables -D $FW_CHAIN -m iprange --src-range 192.168.1.235-192.168.240 -j REJECT
  # Reverse Single IP Mode iptables rules
  #iptables -D $FW_CHAIN -s 192.168.1.235 -o eth0 -j REJECT
  # Remove custom chain name
  #iptables -D FORWARD -i br+ -o eth+ -j $FW_CHAIN
  ...
  if [ "$Version" == "$DLVersion" ]
    then
      ...
      if promptyn " (y/n): "; then
        ...
        curl --silent --retry 3 "https://raw.githubusercontent.com/ViktorJp/RTRMON/master/killmon-$DLVersion.sh" -o "/jffs/scripts/killmon.sh" && chmod a+rx "/jffs/scripts/killmon.sh"
        echo ""
        echo -e "${CCyan} Download successful!${CClear}"
        echo -e "$(date) - KILLMON - Successfully downloaded KILLMON v$DLVersion" >> $LOGFILE
        echo ""
        echo -e "${CYellow} Please exit, restart and configure new options using: 'killmon.sh -setup'.${CClear}"
      ...
      if promptyn " (y/n): "; then
        echo ""
        echo -e "${CCyan} Downloading KILLMON ${CYellow}v$DLVersion${CClear}"
        curl --silent --retry 3 "https://raw.githubusercontent.com/ViktorJp/KILLMON/master/killmon-$DLVersion.sh" -o "/jffs/scripts/killmon.sh" && chmod a+rx "/jffs/scripts/killmon.sh"
        echo ""
        echo -e "${CCyan} Download successful!${CClear}"
        echo -e "$(date) - KILLMON - Successfully downloaded KILLMON v$DLVersion" >> $LOGFILE
        echo ""
...
if [ "$1" == "-h" ] || [ "$1" == "-help" ] || [ "$1" == "-setup" ] || [ "$1" == "-monitor" ] || [ "$1" == "-protect" ]
  then
    clear
  else
    clear
  ...
  # Execute chosen selections
      case "$Selection" in
        ...
        [Ii])
          ...
          echo -e " the rules. KILLMON statistics are now integrated with VPNMON-R2."
